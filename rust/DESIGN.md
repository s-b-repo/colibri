...
**Intended outcome:** a `rust/` cargo workspace producing a `coli-engine` binary that is a
...
```
rust/crates/
  coli-core/     # QT formats (fmt 0..4), Cfg, safetensors index      (↔ c/st.h)
  coli-kernels/  # std::arch int4/int8/int2 + f32 matmul, token-exact  (↔ matmul_qt_ex, glm.c:978)
  coli-io/       # io-uring reactor (I/O lane), slab arena, LRU/pin    (↔ c/uring.h, tier.h)
  coli-cuda/     # -sys FFI to backend_cuda.h + build.rs(nvcc) + wrapper
  coli-model/    # MLA, router, MoE, DSA, MTP + the 3-lane scheduler   (↔ glm.c forward)
  coli-engine/   # binary: clap CLI + stdio serve protocol (drop-in for c/glm)
```
...
- **`c/backend_cuda.cu` kernels via FFI** — validated WMMA/quant/attention over the flat C ABI;
  compiled by `coli-cuda/build.rs`. Rewriting them early buys nothing and costs re-validation.
...
2. M4 throughput proof: run the documented optimized stack config on the same NVIDIA box for both
   engines; the Rust `coli-engine` must exceed the C `tok/s`, with a profiler trace showing GPU,
