//! `QtWeight` — the bridge from `coli-core` container formats to `coli-kernels`
...
//! kernels in `coli-kernels` and are rejected at load time until then.

use coli_core::{Error, QtFmt, QtInfo, SafeTensors};
use coli_kernels::{matmul_i4_from_f32, matmul_i8_from_f32};
